...
public class MainTest {
    ...
    private static void testTempsExecution() {
        ...
        System.out.println("Démarrage du test de la variation du nombre de pistes d'arrivées");
        for (int i = 1; i <= nbPistesMax; i++) {
            Tools.nbPisteArr = i;
            result = testBlockingQueue();
            System.out.println("Nombre de piste(s) : " + i + " -- Temps : " + result + " ms");
        }
        System.out.println("Fin du test de la variation du nombre de pistes d'arrivées !");
    ...
    private static long testBlockingQueue() {
        ...
        for (int i = 0; i < Tools.nbAvion; i++) {
            AvionBlockingQueue avion = new AvionBlockingQueue(airportFrame, codePlane[i], airArr, tarmacLand, tarmacTakeOff, terminal, airDep, Tools.nbAvion, Tools.nbPisteArr, Tools.nbPisteDep, Tools.nbPlace);
    ...
    private static long testCirularBuffer() {
        ...
        for (int i = 0; i < Tools.nbAvion; i++) {
            AvionCircularBuffer avion = new AvionCircularBuffer(airportFrame, codePlane[i], airArr, tarmacLand, tarmacTakeOff, terminal, airDep, Tools.nbAvion, Tools.nbPisteArr, Tools.nbPisteDep, Tools.nbPlace);
            tabThread[i] = new Thread(avion);
            tabThread[i].start();
        }
